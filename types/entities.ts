...
// FIX: Add Point and BoardState type definitions
export type Point = { x: number; y: number };
export type BoardState = Player[][];

// FIX: Add TournamentDefinition interface
...
// FIX: Added missing type definitions to resolve type errors.
...
export type GuildMissionProgressKey = 'checkIns' | 'strategicWins' | 'playfulWins' | 'diamondsSpent' | 'equipmentEnhancements' | 'materialCrafts' | 'equipmentSyntheses' | 'championshipClaims' | 'towerFloor50Conquerors' | 'towerFloor100Conquerors' | 'bossAttempts' | 'epicGearAcquisitions';
...
export interface Guild {
    ...
    missionProgress: Record<GuildMissionProgressKey, number | string[]>;
...
export type SinglePlayerMissionInfo = SinglePlayerStageInfo;
...
export interface User {
    ...
    lastSinglePlayerStageId?: string;
}
...
export interface LiveGameSession {
    ...
    turnSelectionTiebreaker?: 'rps' | 'dice_roll' | 'nigiri';
    ...
    alkkagiRoundSummary?: { round: number; winnerId: string; loserId: string; refillsRemaining: Record<string, number> };
    ...
    curlingRoundSummary?: {
        ...
        scoredStones: Record<number, number>;
    };
    ...
    roundEndConfirmations?: Record<string, boolean | number>;
    ...
    mythicBonuses?: Record<string, Record<MythicStat, number>>;
    lastPlayfulGoldCheck?: Record<string, number>;
    pendingSystemMessages?: { message: string; playerIds: string[] }[];
    ...
    towerItemPurchases?: Partial<Record<'missile' | 'hidden' | 'scan', boolean>>;
