...
export const handleAction = async (volatileState: types.VolatileState, action: types.ServerAction & { userId: string, user: types.User }): Promise<types.HandleActionResult> => {
    ...
    if (['CHALLENGE_USER', 'SEND_CHALLENGE', 'UPDATE_NEGOTIATION', 'ACCEPT_NEGOTIATION', 'DECLINE_NEGOTIATION', 'START_AI_GAME', 'REQUEST_REMATCH'].includes(type)) return handleNegotiationAction(volatileState, action, user);
    ...
    if (type.startsWith('START_TOURNAMENT_SESSION') || type.startsWith('FORFEIT_TOURNAMENT') || type.startsWith('SKIP_TOURNAMENT_END') || type.startsWith('USE_CONDITION_POTION') || type.startsWith('CLEAR_TOURNAMENT_SESSION') || type.startsWith('SAVE_TOURNAMENT_PROGRESS') || type.startsWith('START_TOURNAMENT_ROUND')) return handleTournamentAction(volatileState, action, user);
