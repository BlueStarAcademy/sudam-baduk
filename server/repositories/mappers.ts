    ...
    try {
        ...
        const quests: QuestLog = {
            daily: { 
                ...defaultQuests.daily, 
                ...dailyFromDb, 
                lastReset: dailyFromDb.lastReset ? parseInt(dailyFromDb.lastReset, 10) : 0,
            ...
            weekly: { 
                ...
                ...weeklyFromDb, 
                lastReset: weeklyFromDb.lastReset ? parseInt(weeklyFromDb.lastReset, 10) : 0,
            ...
            monthly: { 
                ...
                ...monthlyFromDb, 
                lastReset: monthlyFromDb.lastReset ? parseInt(monthlyFromDb.lastReset, 10) : 0,
        ...
        const user: User = {
            ...
            chatBanUntil: row.chatBanUntil ? parseInt(row.chatBanUntil, 10) : undefined,
            connectionBanUntil: row.connectionBanUntil ? parseInt(row.connectionBanUntil, 10) : undefined,
            lastActionPointUpdate: parseInt(row.lastActionPointUpdate ?? '0', 10),
            actionPointPurchasesToday: row.actionPointPurchasesToday ?? 0,
            lastActionPointPurchaseDate: parseInt(row.lastActionPointPurchaseDate ?? '0', 10),
            actionPointQuizzesToday: row.actionPointQuizzesToday ?? 0,
            lastActionPointQuizDate: parseInt(row.lastActionPointQuizDate ?? '0', 10),
            ...
            lastWeeklyCompetitorsUpdate: parseInt(row.lastWeeklyCompetitorsUpdate ?? '0', 10),
            lastLeagueUpdate: parseInt(row.lastLeagueUpdate ?? '0', 10),
            monthlyGoldBuffExpiresAt: parseInt(row.monthlyGoldBuffExpiresAt ?? '0', 10),
            mbti: row.mbti ?? null,
            ...
            guildLeaveCooldownUntil: parseInt(row.guildLeaveCooldownUntil ?? '0', 10),
            guildCoins: row.guildCoins ?? 0,
            guildBossAttempts: row.guildBossAttempts ?? 0,
            lastGuildBossAttemptDate: parseInt(row.lastGuildBossAttemptDate ?? '0', 10),
            lastLoginAt: parseInt(row.lastLoginAt ?? '0', 10),
            // FIX: Removed duplicate `date` property from object literal.
            dailyDonations: { gold: 0, diamond: 0, ...dailyDonationsFromDb, date: dailyDonationsFromDb.date ? parseInt(String(dailyDonationsFromDb.date), 10) : 0 },
            // FIX: Removed duplicate `date` property from object literal.
            dailyMissionContribution: { amount: 0, ...dailyMissionContributionFromDb, date: dailyMissionContributionFromDb.date ? parseInt(String(dailyMissionContributionFromDb.date), 10) : 0 },
            guildShopPurchases: ensureObject(safeParse(row.guildShopPurchases, {})),
            appSettings,
            // Tournament progress
            lastNeighborhoodPlayedDate: row.lastNeighborhoodPlayedDate ? parseInt(row.lastNeighborhoodPlayedDate, 10) : undefined,
            ...
            lastNationalPlayedDate: row.lastNationalPlayedDate ? parseInt(row.lastNationalPlayedDate, 10) : undefined,
            ...
            lastWorldPlayedDate: row.lastWorldPlayedDate ? parseInt(row.lastWorldPlayedDate, 10) : undefined,
            ...
            lastChampionshipMatchDate: parseInt(row.lastChampionshipMatchDate ?? '0', 10),
        };
        return user;
    } catch (e) {
...
export const rowToGame = (row: any): LiveGameSession | null => {
    ...
    try {
        ...
        const game: LiveGameSession = {
            ...
            blackTimeLeft: parseFloat(row.blackTimeLeft),
            whiteTimeLeft: parseFloat(row.whiteTimeLeft),
            blackByoyomiPeriodsLeft: row.blackByoyomiPeriodsLeft,
            whiteByoyomiPeriodsLeft: row.whiteByoyomiPeriodsLeft,
            turnDeadline: row.turnDeadline ? parseFloat(row.turnDeadline) : undefined,
            turnStartTime: row.turnStartTime ? parseFloat(row.turnStartTime) : undefined,
            ...
            guessDeadline: row.guessDeadline ? parseFloat(row.guessDeadline) : undefined,
            bids: ensureObject(safeParse(row.bids, undefined), undefined),
            biddingRound: row.biddingRound ?? undefined,
            captureBidDeadline: row.captureBidDeadline ? parseFloat(row.captureBidDeadline) : undefined,
            ...
            basePlacementDeadline: row.basePlacementDeadline ? parseFloat(row.basePlacementDeadline) : undefined,
            komiBids: ensureObject(safeParse(row.komiBids, undefined), undefined),
            komiBiddingDeadline: row.komiBiddingDeadline ? parseFloat(row.komiBiddingDeadline) : undefined,
            komiBiddingRound: row.komiBiddingRound ?? undefined,
            komiBidRevealProcessed: !!row.komiBidRevealProcessed,
            finalKomi: row.finalKomi ? parseFloat(row.finalKomi) : undefined,
            ...
            turnOrderRollDeadline: row.turnOrderRollDeadline ? parseFloat(row.turnOrderRollDeadline) : undefined,
            turnOrderAnimationEndTime: row.turnOrderAnimationEndTime ? parseFloat(row.turnOrderAnimationEndTime) : undefined,
            turnChoiceDeadline: row.turnChoiceDeadline ? parseFloat(row.turnChoiceDeadline) : undefined,
            ...
            alkkagiTurnDeadline: row.alkkagiTurnDeadline ? parseFloat(row.alkkagiTurnDeadline) : undefined,
            alkkagiPlacementDeadline: row.alkkagiPlacementDeadline ? parseFloat(row.alkkagiPlacementDeadline) : undefined,
            ...
            curlingTurnDeadline: row.curlingTurnDeadline ? parseFloat(row.curlingTurnDeadline) : undefined,
            ...
            pausedTurnTimeLeft: row.pausedTurnTimeLeft ? parseFloat(row.pausedTurnTimeLeft) : undefined,
            itemUseDeadline: row.itemUseDeadline ? parseFloat(row.itemUseDeadline) : undefined,
            lastTimeoutPlayerId: row.lastTimeoutPlayerId ?? undefined,
            lastTimeoutPlayerIdClearTime: row.lastTimeoutPlayerIdClearTime ? parseFloat(row.lastTimeoutPlayerIdClearTime) : undefined,
            revealAnimationEndTime: row.revealAnimationEndTime ? parseFloat(row.revealAnimationEndTime) : undefined,
            revealEndTime: row.revealEndTime ? parseFloat(row.revealEndTime) : undefined,
            isAiGame: !!row.isAiGame,
            aiTurnStartTime: row.aiTurnStartTime ? parseFloat(row.aiTurnStartTime) : undefined,
