...
export const getGoLogic = (gameSettings) => {
...
export const processMove = (boardState, move, koInfo, moveIndex, options = {}) => {
    ...
    if (x === -1 || y === -1)
        return { isValid: true, newBoardState: boardState, capturedStones: [], newKoInfo: koInfo };
    if (x >= boardSize || y >= boardSize || x < 0 || y < 0 || boardState[y]?.[x] !== Player.None)
        return { isValid: false, reason: 'Point is not empty', newBoardState: boardState, capturedStones: [], newKoInfo: koInfo };
