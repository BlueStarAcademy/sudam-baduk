...
import { InventoryItem, ItemGrade, EquipmentSlot, CoreStat, SpecialStat, MythicStat, UserWithStatus } from '../types/index.js';
import { gradeStyles, getStarDisplayInfo } from '../utils/itemDisplayUtils.js';
import { CORE_STATS_DATA, SPECIAL_STATS_DATA, MYTHIC_STATS_DATA } from '../constants/index.js';
import { useAppContext } from '../hooks/useAppContext.js';
import { calculateItemStats } from '../utils/statUtils.js';
...
const DetailedItemDisplay: React.FC<DetailedItemDisplayProps> = ({ item, equippedItem, isEquippedItem }) => {
    ...
    return (
        <div className="w-full h-full flex flex-col bg-gray-800/50 rounded-lg p-4 relative">
            <div className="flex items-center justify-between mb-2">
                <h3 className={`font-bold text-xl ${gradeStyles[item.grade]}`}>{item.name}</h3>
                {item.isEquipped && <span className="text-green-400 text-sm font-bold">[장착중]</span>}
            </div>
            <p className="text-gray-400 text-xs mb-3">{item.description}</p>

            <div className="flex items-center justify-center mb-4 relative">
                <img src={gradeBackgrounds[item.grade]} alt={item.grade} className="absolute inset-0 w-full h-full object-cover rounded-md opacity-20" />
                <img src={item.image || '/images/equipments/empty.png'} alt={item.name} className="w-32 h-32 object-contain relative z-10" />
                {item.stars > 0 && (
                    <div className="absolute bottom-0 right-0 bg-black/60 rounded-tl-md px-2 py-1 flex items-center gap-1 z-10">
                        <img src={getStarDisplayInfo(item.stars).starImage} alt="star" className="w-4 h-4" />
                        <span className={`font-bold text-lg ${getStarDisplayInfo(item.stars).numberColor}`}>{item.stars}</span>
                    </div>
                )}
            </div>
            ...
            {item.type === 'equipment' && (
                <div className="space-y-2 flex-grow overflow-y-auto pr-2">
                    <div className="mb-2">
                        <h4 className="font-semibold text-blue-300 border-b border-gray-600 pb-1 mb-1 text-sm">기본 능력치</h4>
                        <div className="space-y-1">
            ...
            {item.type === 'consumable' && item.quantity && (
                <p className="text-gray-300 text-sm text-center mt-auto">수량: {item.quantity}</p>
            )}
            ...
            {!canEquip && item.type === 'equipment' && (
                <div className="absolute inset-0 bg-black/70 flex items-center justify-center text-center text-lg font-bold text-red-400 rounded-lg">
