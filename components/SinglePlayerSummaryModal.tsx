...
import { SINGLE_PLAYER_STAGES, TOWER_STAGES, CONSUMABLE_ITEMS, MATERIAL_ITEMS } from '../constants/index.js';
...
const SinglePlayerSummaryModal: React.FC<SinglePlayerSummaryModalProps> = ({ session, currentUser, onAction, onClose, isTopmost }) => {
    ...
    const handleRetry = () => {
        if (stage && currentUser.actionPoints.current >= stage.actionPointCost) {
            onAction({ type: 'START_SINGLE_PLAYER_GAME', payload: { stageId: session.stageId!, previousGameId: session.id } });
            onClose(false);
        }
    };
    ...
    const handleNextStage = () => {
        if (canTryNext && nextStage) {
            ...
            onAction({ type: 'START_SINGLE_PLAYER_GAME', payload: { stageId: nextStage.id, previousGameId: session.id } });
            onClose(false);
        }
    };
    ...
    return (
        <DraggableWindow title="대국 결과" onClose={() => onClose(true)} windowId="singleplayer-summary" initialWidth={550} isTopmost={isTopmost}>
