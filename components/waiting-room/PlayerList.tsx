...
const PlayerList: React.FC<PlayerListProps> = ({ users, onAction, currentUser, mode, negotiations, onViewUser }) => {
    ...
    const renderUserItem = (user: UserWithStatus, isCurrentUser: boolean) => {
        ...
        const statusInfo = statusDisplay[user.status] || { text: '알 수 없음', color: 'text-gray-500' };
                ...
                {isCurrentUser ? (
                    ...
                    >
                        ...
                        {!['waiting', 'resting'].includes(currentUser.status) && (
                            <option value={currentUser.status} disabled>{(statusDisplay[currentUser.status] && statusDisplay[currentUser.status].text) || currentUser.status}</option>
                        )}
                    </select>
                ) : (
